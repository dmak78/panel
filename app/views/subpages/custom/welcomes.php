    ...
    <div class="grid-item">
      ...
      <div class="dropzone subpages">
        <div class="items<?php e($sortable, ' sortable') ?>" id="visible-children" 
          ...
          <?php echo new Kirby\Panel\Snippet('subpages/custom/welcome_content', array('page' => $page, 'subpage' => $subpage)) ?>
          <?php endforeach ?>
        </div>
      </div>
...
(function() {
  ...
  $('.visibility').on('click', function(e){
    ...
    var id = $item.attr('id');
    var $others = $('.item').not($item);

    $.post(window.location.href, {action: 'sort', id: id, to: 1}, function(data) {
      $.each($others, function(index, other) {
        var other_id = $(other).attr('id');
        $.post(window.location.href, {action: 'hide', id: other_id}, function(other_data) {
